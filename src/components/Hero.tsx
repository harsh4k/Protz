...
export default function Hero() {
  ...
  // Parallax scroll effects for the backgrounds
  const { scrollY } = useScroll();
  
  // Slide typography slower in opposite directions for depth
  const textY1 = useTransform(scrollY, [0, 800], [0, -100]);
  const textY2 = useTransform(scrollY, [0, 800], [0, -60]);
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1600px] mx-auto w-full relative z-10 flex flex-col justify-center mt-auto mb-auto pt-16">
        ...
        {/* Massive Typography - Line 2 (Offset for visual tension) */}
        <motion.div
          style={{ y: textY2 }}
        ...
        {/* Role & Secondary Info (Positioned with extensive negative space) */}
        <div className="mt-[6vw] flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-30">
          ...
          >
            <button 
              ...
              className="font-mono text-[10px] tracking-[0.25em] uppercase border border-[rgba(240,237,232,0.25)] px-6 py-4 hover:bg-[#F0EDE8] hover:text-[#0A0A0A] transition-all cursor-pointer focus:outline-none"
            >
              EXPLORE INDEX ↓
            </button>
          </motion.div>
